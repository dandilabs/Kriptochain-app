    ...
    <div class="container">
        ...
        <form action="{{ route('admin.membership-plans.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="period_month" class="form-control" value="{{ old('period_month') }}">
            </div>
            <div class="mb-3">
                <label>Fitur (satu per baris)</label>
                <textarea name="features" class="form-control" rows="5" required>{{ old('features') }}</textarea>
            </div>
            <div class="mb-3">
                ...
                <select name="promos[]" class="form-control" multiple>
                    @foreach ($promos as $promo)
                        <option value="{{ $promo->id }}"
                            {{ collect(old('promos'))->contains($promo->id) ? 'selected' : '' }}>{{ $promo->name }}
                        </option>
                    @endforeach
                </select>
            </div>
